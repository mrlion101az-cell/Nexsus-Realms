...
public class NexusRealmsPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        VaultHook economy = new VaultHook();
        boolean vaultFound = economy.setup(this);
        if (!vaultFound) {
            getLogger().warning("Vault (or an economy plugin behind it) was not found. "
                    + "Chunk cap upgrades cannot be purchased until an economy is available.");
        }
        ...
        getServer().getPluginManager().registerEvents(new ProtectionListener(teams, land, personalClaims, config), this);

        getCommand("team").setExecutor(new TeamCommand(teams, hints));
        getCommand("realms").setExecutor(new LandCommand(teams, land, config, hints, economy));
        ...
        getServer().getPluginManager().registerEvents(new WandListener(selections, config), this);
        getCommand("redit").setExecutor(new EditCommand(selections, editPermissions, editExecutor, config));
